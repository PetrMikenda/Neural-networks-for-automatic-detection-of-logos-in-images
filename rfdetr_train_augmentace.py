...
# python rfdetr_train_augmentace.py \
#   --seed 42 \
#   --dataset datasets/flickr32 \
#   --project Augmentations_flickr \
#   --models rfdetr_large
...
# ==============================================================================
parser = argparse.ArgumentParser(description="Univerzální trénovací skript pro RF-DETR - Augmentace.")
...
parser.add_argument('--models', nargs='+', default=['rfdetr_large'], help='Seznam modelů (rfdetr_small, rfdetr_large)')
args = parser.parse_args()

# ==============================================================================
# 2. UZAMČENÍ NÁHODY
# ==============================================================================
print(f"Zamykám všechny generátory náhody na seed: {args.seed}")
...
# ==============================================================================
# 4. DEFINICE AUGMENTAČNÍCH PROFILŮ (Albumentations format)
# ==============================================================================
augmentation_profiles_rfdetr = {
    "no_aug": {
        "ColorJitter": {"p": 0.0},
        "Affine": {"p": 0.0},
    ...
    "heavy_aug": {
        "ColorJitter": {
            "hue": 0.1, "saturation": 0.9, "brightness": 0.9, "p": 1.0
        },
        "Affine": {
            "shear": [-20, 20], "rotate": [-45, 45], "p": 1.0
        },
    ...
    "logo_optimized": {
        "ColorJitter": {
            "hue": 0.0, "saturation": 0.0, "brightness": 0.2, "p": 1.0
        },
        "Affine": {
            "scale": [0.5, 1.5], "translate_percent": [-0.1, 0.1], "p": 1.0
        }
    }
}
...
# ==============================================================================
EPOCHS = 100
print(f"STARTUJI TEST: {len(args.models)} modelů x {len(augmentation_profiles_rfdetr)} profilů")

for model_name in args.models:
    if model_name not in model_mapping:
        print(f"Model {model_name} neexistuje v mapování, přeskakuji."); continue
        
    ModelClass = model_mapping[model_name]
    
    for aug_name, aug_config in augmentation_profiles_rfdetr.items():
        run_name = f"{model_name}_{aug_name}_seed{args.seed}"
        os.environ["WANDB_RUN_GROUP"] = f"{model_name}_Hyp3"
        ...
        try:
            model = ModelClass(resolution=640)
            model.train(
